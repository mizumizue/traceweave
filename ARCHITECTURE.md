...
lucid-memories の設計指針（ADR-0008, ADR-0006）に厳格に準拠し、リポジトリルートには製品のガバナンス・全体構成マップ（`README.md`, `ARCHITECTURE.md`, `DEVELOPER_GUIDE.md`）、除外設定（`.gitignore`）、および実行用ラッパー（`bin/`）のみを最前面に配置する。
