...
depends_on: []
tags: [test, integration, graph, layout, highlight, inspector, upstream, downstream]
links: []
---
## Content
...
### Actual Results
- `TraceGraph` を builder の入力オプションへ渡した実ドキュメント結合で、階層レイアウト、トレースパス、種別除外、インスペクター用ノード情報のアサーションが成功した。

### Evidence
`tests/core/traceability-graph-integration.test.ts` の実行結果ログ。
