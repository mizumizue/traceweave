 ...
 * - 関連文書: TC-UT-0006
 */
test('TC-UT-0006: filterMatrixRows - 重要度・工程・充足度ステータス・キーワードの複合フィルター判定が正確に行われること', () => {
  const sampleRows: MatrixRow[] = [
    {
      ...
      testCasesByPhase: {
        unit: [{ id: 'TC-UT-0001', title: 'UT', level: 'unit', method: 'unit_mock', status: 'passed' }],
        integration_internal: [],
        integration_external: [],
        system: [],
        acceptance: [],
      },
      allTestCases: [{ id: 'TC-UT-0001', title: 'UT', level: 'unit', method: 'unit_mock', status: 'passed' }],
    },
    {
    ...
    },
    {
      ...
      testCasesByPhase: {
        unit: [{ id: 'TC-UT-0003', title: 'UT', level: 'unit', method: 'unit_mock', status: 'passed' }],
        integration_internal: [],
        integration_external: [],
        system: [],
        acceptance: [],
      },
      allTestCases: [{ id: 'TC-UT-0003', title: 'UT', level: 'unit', method: 'unit_mock', status: 'passed' }],
    },
  ];
 ...
 * - 関連文書: TC-UT-0006
 */
test('TC-UT-0006: matrixData - トレーサビリティデータのCSV・JSON・Markdownシリアライズが正確に行われること', () => {
  const sampleRows: MatrixRow[] = [
    {
      ...
      testCasesByPhase: {
        unit: [{ id: 'TC-UT-0006', title: 'UT', level: 'unit', method: 'unit_mock', status: 'passed' }],
        integration_internal: [],
        integration_external: [],
        system: [],
        acceptance: [],
      },
      allTestCases: [{ id: 'TC-UT-0006', title: 'UT', level: 'unit', method: 'unit_mock', status: 'passed' }],
    },
  ];
  ...
  assert.equal(
    fullCsv.split('\n')[1],
    '"","REQ-0015","マトリクス表の多軸フィルターおよびデータエクスポート","","medium","100%","SPEC-0015","TC-UT-0006(unit)"'
  );
  assert.ok(fullCsv.includes('"REQ-0015"'));
  assert.ok(fullCsv.includes('"SPEC-0015"'));
  assert.ok(fullCsv.includes('"TC-UT-0006(unit)"'));
  ...
  assert.ok(fullMd.includes('| **REQ-0015**: マトリクス表の多軸フィルターおよびデータエクスポート | - | medium | 100% | `SPEC-0015` | `TC-UT-0006` |'));
});
