 ...
 * - 条件: 外部環境依存手法を持つテストケース（TC-ITb-0002: scenario/FS, TC-ITb-0004: e2e, TC-UAT-0001: manual）を判定
 * - 期待結果: 全て isModifiable が false、reasonCode が 'external_environment_dependency' と判定されること
 * - 関連文書: TC-UAT-0001, ADR-0003, REQ-0009, SPEC-0008
 */
test('TC-UAT-0001: TestCaseInputAnalyzer - Rule 1: 外部環境依存（E2E・手動・シナリオ等）を持つテストケースが入力変更不可として除外されること', () => {
  const parser = new DocParser();

  // TC-ITb-0002: scenario (storage & fs)
  ...
  // TC-ITb-0004: e2e (system)
  const tc6 = parser.parseFile(repositoryPath('docs/test-cases/TC-ITb-0004.md'));
  assert.ok(tc6);
  ...
  // TC-UAT-0001: exploratory_manual (acceptance)
  const tc7 = parser.parseFile(repositoryPath('docs/test-cases/TC-UAT-0001.md'));
  assert.ok(tc7);
 ...
 * - 条件: パラメータファイルを持たない単体テストケース（TC-UT-0001, TC-UT-0003）を判定
...
test('TestCaseInputAnalyzer - Rule 2: パラメータデータセット（parameter_file）を持たないテストケースが入力変更不可として除外されること', () => {
  ...
  // TC-UT-0001: unit test without parameter_file
  const tc1 = parser.parseFile(repositoryPath('docs/test-cases/TC-UT-0001.md'));
  assert.ok(tc1);
  ...
  // TC-UT-0003: circular gauge unit test without parameter_file
  const tc12 = parser.parseFile(repositoryPath('docs/test-cases/TC-UT-0003.md'));
  assert.ok(tc12);
 ...
 * - 条件: パラメータ化された純粋計算テスト（TC-UT-0002: 充足度計算）を判定
...
test('TestCaseInputAnalyzer - Rule 4: 単純入出力の充足度計算テストが入力変更可能（modifiable）として認定されること', () => {
  ...
  // TC-UT-0002-style pure calculation input (kept in memory so this unit test
...
test('TestCaseInputAnalyzer - analyzeAll と summarize によりリポジトリ全体のテストケース入力変更可否サマリーが集計・フォーマット出力できること', () => {
  ...
  assert.ok(formattedText.includes('TC-UT-0002'));
  assert.ok(formattedText.includes('TC-ITb-0007'));
});
