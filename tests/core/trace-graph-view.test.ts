 ...
 * - 関連文書: TC-ITb-0014, REQ-0020, REQ-0021, SPEC-0018
 */
test('TC-ITb-0014: TraceabilityGraphBuilder - 全種別ノードの階層ランク付け・XY座標算出およびエッジ構築が正しく行われること', () => {
  const nodes: DocNode[] = [
    {
      id: 'NEED-0001',
      kind: 'need',
  ...
  assert.ok(specVNode.downstreamIds.includes('TC-UT-0001'));

  const tcVNode = graphData.nodes.find(n => n.id === 'TC-UT-0001')!;
 ...
 * - 関連文書: TC-ITb-0014, REQ-0021, SPEC-0018
 */
test('TC-ITb-0014: TraceabilityGraphBuilder - 選択ノードに基づく上流・下流ハイライト探索および非関連ノードの減衰処理が正しく動作すること', () => {
  const nodes: DocNode[] = [
    {
      id: 'NEED-0001',
      kind: 'need',
  ...
  const tcNode = allHighlighted.nodes.find(n => n.id === 'TC-UT-0001')!;
  ...
  const upTc = upHighlighted.nodes.find(n => n.id === 'TC-UT-0001')!;
  ...
  const downTc = downHighlighted.nodes.find(n => n.id === 'TC-UT-0001')!;
 ...
 * - 関連文書: TC-ITb-0014, REQ-0021, SPEC-0018
 */
test('TC-ITb-0014: TraceabilityGraphBuilder - 循環参照を含むグラフの安全な探索完了および種別フィルターによる絞り込みが機能すること', () => {
  // Test cycles safety
  ...
  // Exclude test_case: only TC-UT-0001 is hidden, NEED-0001 and REQ-0001 remain
  ...
  assert.ok(!tcExcluded.nodes.some(n => n.id === 'TC-UT-0001'));
  // Edges connecting to TC-UT-0001 should also be excluded
 ...
 * - 関連文書: TC-ITb-0014, REQ-0020, REQ-0021, REQ-0022
 */
test('TC-ITb-0014: TraceabilityGraphBuilder - 実際のdocsディレクトリを対象とした全階層レイアウトおよびエッジ接続の結合検証', () => {
