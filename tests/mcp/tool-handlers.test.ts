 ...
 * - 関連文書: TC-ITb-0026, REQ-0029, SPEC-0023
 */
test('TC-ITb-0026: MCP ツール一覧が 7 件の契約どおり公開されること', () => {
  const tools = listMcpTools();
  assert.equal(tools.length, 7);
  assert.deepEqual(tools.map(tool => tool.name), [...MCP_TOOL_NAMES]);
});

test('TC-ITb-0026: get_traceability_summary がサマリ JSON を返すこと', () => {
...
test('TC-ITb-0026: get_requirement_status が未検出要件で ERR_REQUIREMENT_NOT_FOUND を返すこと', () => {
...
test('TC-ITb-0026: check_quality_gaps が passed フィールドを含む JSON を返すこと', () => {
...
test('TC-ITb-0030: get_traceability_matrix が matrix 配列を返すこと', () => {
...
test('TC-ITb-0030: get_catalog が filteredCount を含む JSON を返すこと', () => {
...
test('TC-ITb-0030: get_decisions が adrs と dsns を返すこと', () => {
