 ...
 * - 条件: parameter_file を持つテストケース (TC-ITb-0007) および外部環境依存のテストケース (TC-ITb-0002) をパース後 enrichment
 * - 期待結果: TC-ITb-0007 は外部JSONパラメータが自動読み込みされ、外部ファイル依存としてUI実行対象外(ui_executable: false)となる一方、TC-ITb-0002 も除外判定されること
 * - 関連文書: TC-ITb-0008, REQ-0010, SPEC-0010
 */
test('TC-ITb-0008: DocParser - parameter_file のパースと外部パラメータデータセットの自動ロードおよびUI実行可否判定ができること', () => {
  const parser = new DocParser();
  const tcPath = repositoryPath('docs/test-cases/TC-ITb-0007.md');
  const node = enrichDocNodes([parser.parseFile(tcPath)!])[0];

  assert.ok(node, 'TC-ITb-0007 node should parse successfully');
  assert.equal(node.id, 'TC-ITb-0007');
  assert.equal(node.parameter_file, 'fixtures/test-cases/TC-ITb-0007.json');
  assert.ok(node.parameters, 'node.parameters should be automatically loaded from external json');
  assert.equal(node.parameters.testCaseId, 'TC-ITb-0007');
  assert.ok(node.parameters.patterns.length >= 5);
  assert.equal(node.ui_executable, false, 'TC-ITb-0007 should be excluded because it loads an external dataset');
  ...
  const tc4Path = repositoryPath('docs/test-cases/TC-ITb-0002.md');
  const node4 = enrichDocNodes([parser.parseFile(tc4Path)!])[0];
  assert.equal(node4?.ui_executable, false, 'TC-ITb-0002 should be excluded from UI execution (ui_executable: false)');
 ...
 * - 関連文書: TC-ITb-0008, REQ-0010, SPEC-0010
 */
test('TC-ITb-0008: TestRunnerRegistry - テストピラミッド診断のデータ駆動テストにおいて入力に応じた診断ステータスを正しく判定できること', () => {
...
test('TC-ITb-0008: CLI serve - 外部HTTP APIが対話型テスト実行結果を返すこと', async () => {
...
test('TC-ITa-0003-02: CLI serve - /api/test/run が未知TCで404・UI除外TCで400を返すこと', async () => {
  ...
  try {
    ...
    const unknown = await fetch(`http://127.0.0.1:${port}/api/test/run`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ testCaseId: 'TC-UT-9999', inputs: {} }),
    });
    ...
    const excluded = await fetch(`http://127.0.0.1:${port}/api/test/run`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ testCaseId: 'TC-ITb-0007', inputs: {} }),
    });
