 ...
 * - 条件: 初期ノード REQ-0001 を表示後、依存リンクをクリックして SPEC-0002、TC-0001 へ順次遷移
 * - 期待結果: 履歴スタックが [REQ-0001, SPEC-0002, TC-0001] として蓄積され、戻る操作で SPEC-0002 -> REQ-0001 へ、進む操作で復帰できること
 * - 関連文書: TC-0021, REQ-0013, SPEC-0013
 */
test('TC-0021: NodeDetailModal - 閲覧履歴スタックの蓄積および戻る・進むナビゲーションが正確に行われること', () => {
  let state = { history: ['REQ-0001'], index: 0 };
  state = appendModalHistory(state, 'SPEC-0002');
  state = appendModalHistory(state, 'TC-0001');
  assert.deepEqual(state, { history: ['REQ-0001', 'SPEC-0002', 'TC-0001'], index: 2 });
  ...
  assert.equal(state.history[state.index], 'SPEC-0002');
  state = moveModalHistory(state, 'forward');
  assert.equal(state.history[state.index], 'TC-0001');
  state = moveModalHistory(state, 'forward');
 ...
 * - 関連文書: TC-0021, REQ-0013, SPEC-0013
 */
test('TC-0021: NodeDetailModal - Escapeキーおよび外側クリックハンドラにより安全にクローズ処理が呼び出されること', () => {
 ...
 * - 関連文書: TC-0021, REQ-0013, SPEC-0013
 */
test('TC-0021: NodeDetailModal - ノードID・ファイルパス・Markdown本文のコピーデータ抽出', () => {
