...
- **トレーサビリティ**: 要求（NEED）→ 要件（REQ）→ 詳細仕様（SPEC）→ 設計（DSN）→ テストケース（TC）の双方向チェーンを、Git で管理する Markdown から自動構築します。
- **工程地層とテストピラミッド**: テストを 5 大工程（単体・内結・外結・総合・受入）と手法（Mock、API Contract、Scenario、E2E など）に分類し、層の偏り、結合テストの空洞化、E2E への偏りを診断します。
- **自己適用**: TraceWeave 自身も同じスキーマで定義しており、自らのトレーサビリティと品質をダッシュボードで可視化しています。
